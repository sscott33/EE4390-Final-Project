...
;-------------------------------------------------------------------------------
     ...
     mov.b   #00000100b, &P1DIR         ; P1.2 will be TX output
                                        ; Selected by P1SEL and P1SEL2
                                        ; P1.3, P1.4, P1.5 are pushbutton inputs
     mov.b   #00111000b, &P1REN         ; Enables Pull-up/Pull-down resistors
...
;-----------------------------------------------------------------------------------
RELOAD_R10:
     mov     #8, R5              ; Move the number 8 into the counter

WAITING_TO_SEND:                 ; Testing if the "SEND" has been pushed
     bit.b   #00001000b, &P1IN   ; Has "SEND" button been pushed
     jnz     WAITING_TO_SEND     ; Keep checking the "SEND" button
;------------------------------------------------------------------------------------
...
;------------------------------------------------------------------------------------
     mov     R10, &UCA0TXBUF   ; R10 moved into the Transmit Buffer and sent out

     jmp     RELOAD_R10        ; Goto the top and reload the next 8 Bits into R10
;-------------------------------------------------------------------------------
